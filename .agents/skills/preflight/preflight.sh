...
cd "$(dirname "$0")/../../.." || exit 0   # repo root; script lives in .agents/skills/preflight/
...
if upstream=$(git rev-parse --abbrev-ref '@{upstream}' 2>/dev/null); then
  ...
  [ "$ahead" != 0 ] && add "$ahead commit(s) on $branch not pushed to $upstream"
  [ "$behind" != 0 ] && add "$branch is $behind commit(s) behind $upstream (fetch age unknown — consider git fetch)"
else
  add "branch $branch has no upstream configured"
fi
...
[ "$n_mod" != 0 ] || [ "$n_untracked" != 0 ] && \
  add "working tree: $n_mod modified/staged, $n_untracked untracked path(s) (normal mid-work; commit per workstream when done)"
...
while IFS= read -r d; do
  ...
  if [ -f "$d/compose.yml" ] || [ -f "$d/docker-compose.yml" ]; then
    add "unregistered stack: untracked directory $d/ contains a compose file"
  fi
...
if running=$(timeout 5 docker ps --format '{{.Names}}' 2>/dev/null); then
  ...
  [ -n "$unknown" ] && add "running container(s) named in no tracked compose file:$unknown"
else
  add "docker ps unavailable (daemon down or >5s timeout) — container check skipped"
fi

# --- versioned git hooks wired? ----------------------------------------------
hooks_path=$(git config core.hooksPath 2>/dev/null || true)
if [ "$hooks_path" != "ops/git-hooks" ]; then
  add "git hooks unwired for this clone — run: git config core.hooksPath ops/git-hooks"
fi
