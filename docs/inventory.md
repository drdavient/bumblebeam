...
| Syncthing `syncthing/config/` (device keys, API key) | Stateful application data | No | Yes | Losing it changes the device ID — every peer must re-pair |
| Syncthing `syncthing/data/` (shared content, e.g. phone folders) | Excluded/reproducible | No | No | Regenerated from Plex collections by `sync-sdcard.sh --dest` |
