...
| P1 | Pin shared Docker network identity | Done | Preserve current subnets while stabilising bridge names | `gluetun-net`=`172.18.0.0/16` on `br-gluetun`; `traefik-net`=`172.26.0.0/16` on `br-traefik` | `ops/networks/compose.yml`, `ops/networks/ensure-networks.sh`; host verification after recreation |
